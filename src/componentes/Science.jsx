...
import video from "/public/img/video2.mp4";
...
export const Science = () => {
  ...
  useEffect(() => {
    const url =
      "https://api.yadio.io/market/ads?currency=ARS&side=buy&limit=20";
    const peticion = fetch(url);
    peticion
      .then((respons) => respons.json())
      .then((lectura) =>
        setServidor(
          lectura.ads.map((moneda) => (
            <div key={moneda.ad_url} className="text-center mt-5 border-4 border-gray-400 p-2 text-slate-300 ">
              <h3>Price: {moneda.price} </h3>
              <p>
                <a
                  href={moneda.ad_url}
                ...
                >
                  ...
                  <span className="w-full">{moneda.ad_url}</span>
                  <svg
